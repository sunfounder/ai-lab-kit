...
The circuit uses a relay module connected to a GPIO pin. The relay module typically has three pins: VCC (power), GND (ground), and IN (control signal). When the GPIO pin outputs a HIGH signal, the relay activates; when it outputs a LOW signal, the relay deactivates.
...
1. Connect the VCC pin of the relay module to 3.3V on the Raspberry Pi
2. Connect the GND pin of the relay module to GND on the Raspberry Pi
3. Connect the IN pin of the relay module to GPIO 17 on the Raspberry Pi
