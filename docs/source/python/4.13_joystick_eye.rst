...
4.13 OEil controle par joystick
=========================================

**Introduction**

Dans cette lecon, vous allez construire un **OEil controle par joystick**, affiche sur un ecran OLED SSD1306 128×64.
Un joystick analogique a deux axes fournit un mouvement X/Y, lu a l’aide des canaux ADC du Fusion HAT+.
L’OLED affiche un “oeil” stylise dont la pupille suit en douceur la direction du joystick.

Le projet demontre :

- La lecture d’entree analogique multi-axes
- La normalisation fluide des valeurs et la gestion de zone morte
- Les contraintes geometriques utilisant une ellipse
- Le rendu graphique en temps reel sur un OLED monochrome

En deplacant le joystick, l’iris et la pupille glissent a l’interieur de l’oeil sans franchir la limite exterieure de la sclerotique.
...
#. Lorsque le script s’execute :
   ...
   * Deplacer le joystick a gauche/droite/haut/bas deplace la pupille en consequence
   * Le mouvement de la pupille est lisse et contraint a l’interieur d’un oeil elliptique
   * L’affichage se met a jour a environ **50 FPS**
...
1. **Lecture analogique du joystick**

   Deux canaux ADC lisent les positions X et Y ``(0..4095)``.
   Le script convertit ces valeurs en mouvement normalise ``[-1..1]`` avec :

   - Gestion de zone morte
   - Inversion de direction (haut = Y positif)
   - Lissage utilisant une moyenne mobile exponentielle (EMA)
...
2. **Geometrie de l'oeil**

   L'oeil est dessine comme :

   - Une ellipse pour la sclerotique (blanc)
   - Une bordure exterieure optionnelle
   - Un anneau d'iris optionnel
   - Une pupille noire
...
3. **Contrainte du mouvement de la pupille**

   Le centre de la pupille est contraint par l'equation d'ellipse :

   ``(px/MAX_X)^2 + (py/MAX_Y)^2 <= 1``

   Si l'entree pousse la pupille en dehors de l'oeil, le script la projette en arriere sur la limite de l'ellipse.
...
4. **Rendu**
   ...
   - L'arriere-plan de l'oeil est redessine
   - La position de la pupille est calculee
   - Le tampon d'image est envoye a l'OLED
   - L'affichage se rafraichit a ~50 FPS

5. **Boucle principale**

   Lit en continu le joystick → normalise → calcule la pupille → redessine l'oeil → met a jour l'affichage.

----------------------------------------------

**Depannage**

- **Le mouvement du joystick semble saccade**

  - Augmentez ``SMOOTH_ALPHA``
  - Augmentez ``DEADZONE``

- **L'oeil semble etire ou trop petit**
...
- **Les axes du joystick sont inverses**

  Echangez les canaux ADC ou inversez les signes.

----------------------------------------------

**Essayez par vous-meme**

1. **Animation de clignement**

   Ajoutez une ouverture/fermeture temporisee des paupieres.

2. **Mode sourcil fache/heureux**

   Dessinez des sourcils stylises reagissant a l'entree du joystick.

3. **Jeu de suivi de point**

   Ajoutez un point cible mobile et faites en sorte que l'oeil le “suive”.

4. **Ajouter un bouton pour basculer les modes**

   Basculez entre les styles d'oeil : oeil de robot, oeil de dessin anime, oeil de chat.

5. **Ajouter un affichage multi-oeil**

   Affichez deux yeux qui bougent ensemble ou independamment.

Ces extensions transforment le simple oeil a joystick en une puissante demonstration d'animation et d'interaction OLED.
