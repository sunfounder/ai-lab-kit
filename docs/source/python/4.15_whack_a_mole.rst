.. include:: /index.rst
   :start-after: start_hello_message
   :end-before: end_hello_message


.. _py_whack_a_mole:

4.15 Jeu de Whack-a-Mole
=================================

**Introduction**

Dans cette lecon, vous allez construire un **jeu de Whack-a-Mole** en utilisant le Fusion HAT+.
Quatre LEDs pilotees par PWM agissent comme des “taupes”, s’estompant en douceur sur un cycle d’une seconde.
Votre objectif est d’appuyer sur le bon bouton lorsque sa LED correspondante s’allume. Un buzzer fournit un retour audio :

- **Trou correct → bip aigu court**
- **Mauvais trou → GAME OVER (bip grave)**

Un affichage OLED SSD1306 montre votre **score actuel** pendant le jeu et un **score final** en fin de partie.

Ce projet combine la modulation de luminosite PWM, la detection de front des boutons, les boucles de temporisation et une machine a etats de jeu simple.

----------------------------------------------

**Ce dont vous aurez besoin**

Les composants suivants sont necessaires pour ce projet :

.. list-table::
    :widths: 30 20
    :header-rows: 1

    *   - PRESENTATION DU COMPOSANT
        - LIEN D’ACHAT
    *   - :ref:`cpn_wires`
        - |link_wires_buy|
    *   - :ref:`cpn_led`
        - |link_led_buy|
    *   - :ref:`cpn_button`
        - |link_button_buy|
    *   - :ref:`cpn_buzzer`
        - |link_passive_buzzer_buy|
    *   - :ref:`cpn_oled`
        - \-
    *   - :ref:`cpn_fusion_hat`
        - \-
    *   - Raspberry Pi
        - \-

----------------------------------------------

**Schema de cablage**

Reportez-vous au schema de cablage pour assembler les composants :

.. image:: img/fzz/4.15_whack_a_mole_bb.png
   :width: 80%
   :align: center

----------------------------------------------

**Etapes de configuration**

#. Installez les bibliotheques requises :

   .. raw:: html

      <run></run>

   .. code-block:: shell

      sudo pip3 install adafruit-circuitpython-ssd1306 --break

#. Executez le jeu depuis le repertoire ``ai-lab-kit`` :

   .. raw:: html

      <run></run>

   .. code-block:: shell

      cd ~/ai-lab-kit/python/
      sudo python3 4.15_WhackAMole.py

#. Lorsque le script s’execute :

   * Une LED s’allume puis s’eteint progressivement (onde triangulaire).
   * Appuyez **uniquement sur le bouton de la LED allumee** dans la fenetre d’une seconde :

     - Correct → le score augmente, la taupe suivante apparait
     - Incorrect → le buzzer joue un son d’erreur → **GAME OVER**
   * L’OLED affiche uniquement le score actuel (en jeu)
   * L’OLED affiche “GAME OVER” + le score final en cas de perte
   * Appuyez sur Ctrl+C pour quitter

----------------------------------------------

**Code**

Here is the full Python script for the Whack-a-Mole game:

.. raw:: html

   <run></run>

.. code-block:: python

   #!/usr/bin/env python3
   # -*- coding: utf-8 -*-

   """
   Whack-a-Mole using fusion_hat:
   - 4 PWM LEDs (smooth fade in/out; whole cycle = 1.0s)
   - 4 buttons via fusion_hat.Pin (PULL_DOWN; pressed=1)
   - Buzzer via fusion_hat.Buzzer(PWM(...)) for short feedback tone
   - SSD1306 OLED shows only the current hit count during game
   - If a non-lit button is pressed -> GAME OVER with final score
   """

   import time
   import random
   from typing import List, Optional

   # ---- fusion_hat GPIO / PWM / Buzzer ----
   from fusion_hat.pwm import PWM
   from fusion_hat.pin import Pin, Mode, Pull
   from fusion_hat.modules import Buzzer

   # ---- OLED ----
   from PIL import Image, ImageDraw, ImageFont
   import adafruit_ssd1306
   import board

   # ===================== Port map (EDIT HERE) =====================
   # 4 PWM LED ports of Fusion HAT+
   LED_PORTS = ['P0', 'P1', 'P2', 'P3']

   # 4 button pins (BCM numbering). Example here uses 17/4/27/22.
   # Buttons are configured as PULL_DOWN, so pressed -> value==1
   BTN_PINS = [17, 4, 27, 22]

   # Buzzer on a PWM-capable Fusion HAT+ port
   BUZZER_PORT = 'P4'
   # ===============================================================

   # ===================== OLED setup =====================
   WIDTH, HEIGHT = 128, 64
   i2c = board.I2C()
   oled = adafruit_ssd1306.SSD1306_I2C(WIDTH, HEIGHT, i2c, addr=0x3C)
   oled.fill(0)
   oled.show()
   image = Image.new("1", (WIDTH, HEIGHT))
   draw = ImageDraw.Draw(image)
   font = ImageFont.load_default()

   def draw_score(score: int):
      """Render in-game screen: only the current score."""
      draw.rectangle((0, 0, WIDTH, HEIGHT), outline=0, fill=0)
      msg = f"Score: {score}"
      tw, th = font.getbbox(msg)[2:]
      draw.text(((WIDTH - tw) // 2, (HEIGHT - th) // 2), msg, font=font, fill=255)
      oled.image(image)
      oled.show()

   def draw_game_over(score: int):
      """Render GAME OVER screen with final score."""
      draw.rectangle((0, 0, WIDTH, HEIGHT), outline=0, fill=0)
      title = "GAME OVER"
      tw, th = font.getbbox(title)[2:]
      draw.text(((WIDTH - tw) // 2, 8), title, font=font, fill=255)
      line = f"Score: {score}"
      lw, lh = font.getbbox(line)[2:]
      draw.text(((WIDTH - lw) // 2, 8 + th + 8), line, font=font, fill=255)
      oled.image(image)
      oled.show()

   # ===================== Hardware objects =====================
   # LEDs (PWM)
   leds = [PWM(p) for p in LED_PORTS]

   def led_set(idx: int, duty_pct: float):
      """Set LED brightness (0..100%)."""
      duty = max(0.0, min(100.0, float(duty_pct)))
      leds[idx].pulse_width_percent(duty)

   def all_leds_off():
      for i in range(len(leds)):
         leds[i].pulse_width_percent(0)

   # Buttons (Pin, PULL_DOWN => pressed=1)
   buttons = [Pin(pin, mode=Mode.IN, pull=Pull.DOWN) for pin in BTN_PINS]

   def read_buttons() -> List[int]:
      """Read all buttons; return their current logic levels (0/1)."""
      return [b.value() if callable(getattr(b, "value", None)) else int(b.value) for b in buttons]

   # Buzzer (tonal)
   tb = Buzzer(PWM(BUZZER_PORT))

   def beep_hit():
      """Short bright tone for a correct hit."""
      tb.play('A5', 0.08)  # A5 ~ 880 Hz short chirp
      tb.off()

   def beep_miss():
      """Lower/longer tone for a miss (game over)."""
      tb.play('E4', 0.25)  # E4 ~ 330 Hz
      tb.off()

   # ===================== LED fade (1s total) =====================
   def triangle01(u: float) -> float:
      """
      Triangle wave in [0,1], rising 0->1 over first half, then 1->0.
      u in [0,1].
      """
      return 2*u if u < 0.5 else 2*(1.0 - u)

   def fade_led_with_button_check(active_idx: int,
                                 prev_states: List[int],
                                 duration: float = 1.0,
                                 step: float = 0.01,
                                 gamma: float = 1.8) -> (bool, Optional[List[int]]):
      """
      Fade one LED (active_idx) with triangle brightness over 'duration' seconds.
      While fading, poll buttons at 'step' interval:
         - If matching button pressed (rising edge) -> immediately turn off LED, return (True, new_states).
         - If non-matching button pressed (rising edge) -> game over: return (False, new_states) and let caller handle.
      If no press -> return (None, new_states) to continue game.
      Returns:
         (hit, states)
         hit=True  -> correct hit
         hit=False -> wrong button (game over)
         hit=None  -> no button pressed in this cycle
      """
      t0 = time.monotonic()
      states = prev_states[:]

      while True:
         t = time.monotonic() - t0
         if t > duration:
               break

         # Triangle brightness 0..1 then 1..0
         u = triangle01(t / duration)
         # Gamma correction for perceived linearity
         level = u ** gamma
         duty = 3 + 97 * level   # keep a tiny floor so LED is visible at the start

         led_set(active_idx, duty)

         # Poll buttons and detect rising edges
         curr = read_buttons()
         for i, (p, c) in enumerate(zip(states, curr)):
               if p == 0 and c == 1:  # rising edge = just pressed
                  if i == active_idx:
                     # Correct hit
                     all_leds_off()
                     beep_hit()
                     return True, curr
                  else:
                     # Wrong hole -> game over
                     all_leds_off()
                     beep_miss()
                     return False, curr
         states = curr
         time.sleep(step)

      # Finished fade with no press
      all_leds_off()
      return None, states

   # ===================== Game loop =====================
   def pick_next(prev_idx: Optional[int]) -> int:
      """Pick a random index different from prev_idx."""
      choices = list(range(4))
      if prev_idx in choices:
         choices.remove(prev_idx)
      return random.choice(choices)

   def main():
      random.seed(time.time())
      score = 0
      last_idx = None
      states = read_buttons()  # baseline for edge detection

      draw_score(score)

      while True:
         # Choose which LED lights next
         idx = pick_next(last_idx)
         last_idx = idx

         # Run a 1.0s fade cycle, checking button presses
         hit, states = fade_led_with_button_check(idx, states, duration=1.0, step=0.01, gamma=1.8)

         if hit is False:
               # Wrong button pressed -> Game Over
               draw_game_over(score)
               break
         elif hit is True:
               # Correct hit -> score++
               score += 1
               draw_score(score)
         else:
               # No press during this mole -> no score change; continue
               pass

      # stop all outputs
      all_leds_off()

   if __name__ == "__main__":
      try:
         main()
      except KeyboardInterrupt:
         all_leds_off()
         oled.fill(0)
         oled.show()
         tb.off()
         print("\nExited.")


----------------------------------------------

**Comprendre le code**

1. **Animation de fondu des LEDs**

   Chaque LED effectue un fondu en onde triangulaire d'une seconde :

   - La luminosite augmente de 0 a 100%
   - Puis diminue de 100% a 0
   - La correction gamma (``u ** gamma``) ameliore la linearite visuelle

   .. code-block:: python

      def fade_led_with_button_check(active_idx: int,
                                    prev_states: List[int],
                                    duration: float = 1.0,
                                    step: float = 0.01,
                                    gamma: float = 1.8) -> (bool, Optional[List[int]]):
         ...

2. **Detection de front des boutons**

   ``read_buttons()`` lit les quatre boutons.
   Un **front montant** (``previous=0, current=1``) signifie “bouton vient d'etre presse”.

   - Si le bouton presse correspond a la LED allumee → **coup correct**
   - Sinon → **mauvais coup → GAME OVER**

   .. code-block:: python

      def read_buttons() -> List[int]:
         “””Read all buttons; return their current logic levels (0/1).”””
         return [b.value() if callable(getattr(b, “value”, None)) else int(b.value) for b in buttons]



3. **Retour sonore du buzzer**

   - ``A5`` bref gazouillis = succes
   - ``E4`` son plus long = echec / game over

   .. code-block:: python

      # Buzzer (tonal)
      tb = Buzzer(PWM(BUZZER_PORT))

      def beep_hit():
         “””Short bright tone for a correct hit.”””
         tb.play('A5', 0.08)  # A5 ~ 880 Hz short chirp
         tb.off()

      def beep_miss():
         “””Lower/longer tone for a miss (game over).”””
         tb.play('E4', 0.25)  # E4 ~ 330 Hz
         tb.off()



4. **Boucle de jeu**

   - Choisit aleatoirement l'une des quatre LEDs (jamais la meme deux fois de suite)
   - Execute un cycle de fondu, en verifiant les pressions sur les boutons
   - Met a jour le score ou termine le jeu

   .. code-block:: python

      while True:
         # Choose which LED lights next
         idx = pick_next(last_idx)
         last_idx = idx

         # Run a 1.0s fade cycle, checking button presses
         hit, states = fade_led_with_button_check(idx, states, duration=1.0, step=0.01, gamma=1.8)

         ...


5. **Affichage OLED**

   - Affiche uniquement le score pendant le jeu
   - Affiche GAME OVER + le score final lorsque le jeu se termine

   .. code-block:: python

      def draw_score(score: int):
         “””Render in-game screen: only the current score.”””
         draw.rectangle((0, 0, WIDTH, HEIGHT), outline=0, fill=0)
         msg = f”Score: {score}”
         tw, th = font.getbbox(msg)[2:]
         draw.text(((WIDTH - tw) // 2, (HEIGHT - th) // 2), msg, font=font, fill=255)
         oled.image(image)
         oled.show()

         ...



----------------------------------------------

**Depannage**

- **Les boutons n'enregistrent pas les pressions**

  - Assurez-vous que les boutons sont correctement cabbles en utilisant la numerotation des broches BCM
  - Confirmez que ``PULL_DOWN`` est applique pour que l'etat presse = 1
  - Verifiez le cablage de la masse

- **Les LEDs scintillent au lieu de s'estomper en douceur**

  - Reduisez le temps ``step`` pour des mises a jour plus fluides (par exemple, 0.005)
  - Assurez-vous que les ports PWM prennent en charge les mises a jour correctes du cycle de service

- **Le jeu se termine immediatement sans rien presser**

  - Une broche de bouton flottante peut etre lue comme 1
  - Assurez-vous que les resistances de tirage vers le bas sont correctement configurees

- **Le score n'augmente pas**

  - Verifiez que l'index de la LED correspond a l'index du bouton
  - Assurez-vous que les ports LED et les broches des boutons sont dans le bon ordre

----------------------------------------------

**Essayez par vous-meme**

1. **Mode de vitesse variable**

   Raccourcissez progressivement la duree du fondu pour rendre le jeu plus difficile.

2. **Mode multi-LED**

   Allumez deux LEDs a la fois - le joueur doit frapper l'une ou l'autre.

3. **Systeme de vies**

   Autorisez 3 erreurs avant le game over.

4. **Stockage du meilleur score**

   Sauvegardez le meilleur score dans un fichier et affichez-le au demarrage.

5. **Ecran titre anime**

   Ajoutez un ecran de presentation avant le debut du jeu.

Ces idees peuvent transformer Whack-a-Mole en une experience d'arcade complete !
