...
(Exemple) Animal de compagnie numerique
==============================

**Introduction**

Creez un **Animal de compagnie numerique** interactif qui vit sur un ecran OLED et communique par la voix ! Ce projet combine la reconnaissance vocale, la conversation IA, la synthese vocale et le retour visuel pour creer un compagnon virtuel avec sa propre personnalite, ses emotions et ses besoins. L'animal numerique propose :

1. **Interaction vocale** : Parlez a votre animal en utilisant la reconnaissance vocale (STT)
2. **Personnalite IA** : Alimente par OpenAI GPT-4o avec des emotions personnalisees, vous pouvez choisir d'autres LLM.
3. **Affichage emotionnel** : Montre l'humeur en utilisant des emoticones textuelles (kaomoji)
4. **Systeme d'etat** : Niveaux de faim et d'energie qui changent au fil du temps
5. **Retour visuel** : L'ecran OLED affiche l'humeur et l'etat de l'animal
6. **Reponses vocales** : L'animal repond en utilisant une synthese vocale naturelle
...
Votre animal numerique se souvient des conversations, a des etats emotionnels et repond differemment en fonction de ses besoins - creant ainsi une experience de compagnon veritablement interactive !
...
Connectez les composants a votre Raspberry Pi :
...
#. Interagissez avec votre animal

   Lorsque le script demarre :

   * L'OLED affiche un ecran d'accueil avec le nom de votre animal.
   * Un affichage d'etat apparait montrant l'humeur, l'energie et la faim.
   * Le systeme commence a ecouter votre voix.

   Vous pouvez parler naturellement a votre animal, par exemple :
   ...
   * "Tell me a story!"
   ...
   Votre animal repond avec :

   * Une sortie vocale via les haut-parleurs
   * Un affichage emotionnel sur l'OLED
   * Des mises a jour d'etat basees sur votre interaction

#. Quitter le programme

   * Dites "stop" pour terminer l'interaction vocale.
   * Appuyez sur ``Ctrl+C`` pour quitter completement.
...
Voici le script Python complet pour l'Animal de compagnie numerique :
...
1. Reconnaissance vocale (STT)
   ...
   Le systeme utilise Vosk pour la reconnaissance vocale avec des capacites de streaming pour un retour en temps reel :
...
2. Systeme de personnalite IA

   L'animal a une personnalite dynamique avec des etats emotionnels geres via kaomoji :
...
3. Instructions LLM dynamiques

   Les instructions de l'IA sont mises a jour en fonction de l'etat actuel de l'animal et des souvenirs :
...
4. Systeme de gestion d'etat

   Un thread en arriere-plan gere les besoins et l'etat emotionnel de l'animal :
...
5. TTS pilote par les emotions

   La synthese vocale s'adapte en fonction de l'humeur actuelle de l'animal :
...
6. Gestion de l'affichage OLED

   Plusieurs modes d'affichage pour differents etats :
...
7. Changements d'etat interactifs

   Les interactions de l'utilisateur affectent l'etat de l'animal :
...
8. Systeme de memoire

   Garde la trace des conversations recentes :
...
9. Analyse des reponses

   Extrait l'humeur des reponses de l'IA et met a jour l'etat de l'animal :
...
10. Boucle d'interaction principale

    Coordonne tous les composants dans un flux de travail propre :
...
- Entree audio non detectee

  - Executez ``sudo /opt/setup_fusion_hat_audio.sh`` pour reconfigurer l'audio

- L'affichage OLED ne s'affiche pas

  - Verifiez la connexion I2C : ``fusion_hat scan_i2c`` (devrait afficher 0x3C)
  - Verifiez que l'OLED est alimente (3,3 V ou 5 V selon le modele)
  - Assurez-vous de l'adresse I2C correcte dans le code (0x3C ou 0x3D)

- Le TTS ne fonctionne pas

  - Verifiez que la cle API OpenAI dispose de credits TTS
  - Assurez-vous de la connexion Internet pour les appels API
  - Executez ``sudo /opt/setup_fusion_hat_audio.sh`` pour reconfigurer l'audio

- Reconnaissance vocale imprecise

  - Parlez clairement et a un volume modere
  - Reduisez le bruit de fond
  - Ajustez le gain du microphone : ``alsamixer``
  - Essayez differents modeles de langue

- Reponses IA trop lentes

  - Verifiez la vitesse de la connexion Internet
  - Reduisez la complexite des reponses dans les instructions
  - Utilisez un modele OpenAI plus rapide (gpt-3.5-turbo)

- Les barres d'energie/faim ne se mettent pas a jour

  - Verifiez que le thread d'etat est en cours d'execution
  - Verifiez que l'ecran OLED est connecte
  - Recherchez les messages d'erreur dans la console

- L'animal ne se souvient pas des conversations

  - La liste de memoire ne conserve que les 10 dernieres conversations
  - Verifiez que les souvenirs sont correctement ajoutes
  - Assurez-vous que le texte de memoire est transmis au LLM

----------------------------------------------

Ce projet d'animal numerique demontre la puissance de la combinaison de plusieurs technologies d'IA (STT, LLM, TTS) avec des interfaces materielles pour creer des experiences engageantes, emotionnelles et interactives. C'est un exemple parfait de la facon dont l'IA peut creer des connexions significatives grace a la technologie !